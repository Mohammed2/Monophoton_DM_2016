...
void createWorkspaces_Pt_HaloFit(){
  bool connectWZ = true;
  
  std::vector<string> sample_names_DM_LO;
  std::vector<string> sample_names_DM_NLO_pta130;
  std::vector<string> sample_names_ADD;
  std::vector<string> sample_names_DM_EWK;
  sample_names_DM_LO.clear();
  sample_names_DM_NLO_pta130.clear();
  sample_names_ADD.clear();
  sample_names_DM_EWK.clear();
  std::vector<float> signal_multipliers_DM_LO;
  std::vector<float> signal_multipliers_DM_NLO_pta130;
  std::vector<float> signal_multipliers_ADD;
  std::vector<float> signal_multipliers_DM_EWK;
  signal_multipliers_DM_LO.clear();
  signal_multipliers_DM_NLO_pta130.clear();
  signal_multipliers_ADD.clear();
  signal_multipliers_DM_EWK.clear();
  ...
  for(int i = 0; i < sample_names_DM_NLO_pta130.size(); i++){
    if(i == 0)
      cout<<"DM_NLO_pta130_samples_and_multipliers = {"<<"'"<<sample_names_DM_NLO_pta130[i]<<"': '"<<signal_multipliers_DM_NLO_pta130[i]<<"',"<<endl;
    else if(i == sample_names_DM_NLO_pta130.size()-1)
      cout<<"'"<<sample_names_DM_NLO_pta130[i]<<"': '"<<signal_multipliers_DM_NLO_pta130[i]<<"'}"<<endl;
    else
      cout<<"'"<<sample_names_DM_NLO_pta130[i]<<"': '"<<signal_multipliers_DM_NLO_pta130[i]<<"',"<<endl;
  }
  ...
  for(int i = 0; i < sample_names_ADD.size(); i++){
    if(i == 0)
      cout<<"ADD_samples_and_multipliers = {"<<"'"<<sample_names_ADD[i]<<"': '"<<signal_multipliers_ADD[i]<<"',"<<endl;
    else if(i == sample_names_ADD.size()-1)
      cout<<"'"<<sample_names_ADD[i]<<"': '"<<signal_multipliers_ADD[i]<<"'}"<<endl;
    else
      cout<<"'"<<sample_names_ADD[i]<<"': '"<<signal_multipliers_ADD[i]<<"',"<<endl;
  }
  ...
  for(int i = 0; i < sample_names_DM_EWK.size(); i++){
    if(i == 0)
      cout<<"DM_EWK_samples_and_multipliers = {"<<"'"<<sample_names_DM_EWK[i]<<"': '"<<signal_multipliers_DM_EWK[i]<<"',"<<endl;
    else if(i == sample_names_DM_EWK.size()-1)
      cout<<"'"<<sample_names_DM_EWK[i]<<"': '"<<signal_multipliers_DM_EWK[i]<<"'}"<<endl;
    else
      cout<<"'"<<sample_names_DM_EWK[i]<<"': '"<<signal_multipliers_DM_EWK[i]<<"',"<<endl;
  }
}